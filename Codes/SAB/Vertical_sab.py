...
if dia_horas == 'dia':
    sab_tipo1 = pd.read_pickle(PATH_sabs + f'SAB_tipo1_final_{fecha_sel}.pkl')
    forma_sab_1 = pd.read_pickle(PATH_sabs + f'SAB_tipo1_definitivo_{fecha_sel}.pkl')
elif dia_horas == 'hora':
    sab_tipo1 = pd.read_pickle(PATH_sabs + f'SAB_tipo1_final_{fecha_sel}_{hora_ini_str}-{hora_fin_str}.pkl')
    forma_sab_1 = pd.read_pickle(PATH_sabs + f'SAB_tipo1_definitivo_{fecha_sel}_{hora_ini_str}-{hora_fin_str}.pkl')
...
DT_real = pd.read_pickle(PATH_TRAFICO_CELDA + f'DF_T_REAL_CELDA_{fecha_sel}.pkl')
...
df_fl.to_pickle(PATH_sabs + f'FL_tipo2_{fecha_sel}.pkl')
df_fl.to_csv(PATH_sabs + f'FL_tipo2_{fecha_sel}.csv', index=False)
print("Guardado")
...
#%%
# 3. Crear la figura con 2 subgráficos (1 fila, 2 columnas)
fig, axes = plt.subplots(1, 2, figsize=(12, 8), sharey=True)
fig.suptitle('Distribución de Niveles de Vuelo ', fontsize=16, fontweight='bold')
...
# 4. Ajustes visuales para los 2 gráficos
for ax in axes:
    ...
    ax.set_ylim(100, 500) # Fijar estrictamente el eje Y visual entre 320 y 480
...
for item in FL_tipo1:
    ...
    if min_fl is not None and max_fl is not None:
        ...
        if not fila_sab1.empty:
            ...
            espesor = (max_fl+30 - (min_fl-30)) * escala_z
...
fl_separate = 345 # nivel de vuelo de separación de sector

option = input("¿Qué opción del SAB 2 deseas visualizar? (1/2): ").strip().lower()
...
for idx, row in df_fl.iterrows():
    ...
    if not fila_sab2.empty:
        ...
        if option == '1':
            espesor = (FL_max_2 + 50 - (FL_min_2 - 50)) * escala_z
            if espesor == 0: espesor = 0.1 * escala_z # Dar un espesor mínimo si min_fl == max_fl
            base_z = FL_min_2 * escala_z
            
            poligonos = geometria.geoms if isinstance(geometria, MultiPolygon) else [geometria]
            for p in poligonos:
                try:
                    modelo_sab2 = trimesh.creation.extrude_polygon(p, height=espesor)
                    modelo_sab2.apply_translation([0.0, 0.0, base_z])
                    modelo_sab2.visual.face_colors = [100, 255, 100, 180] # Verde semitransparente
                    objetos_escena.append(modelo_sab2)
                except:
                    pass
        elif option == '2':
            espesor_l = (fl_separate-0) * escala_z
            espesor_u = (660-fl_separate) * escala_z
                
            base_z_u = fl_separate * escala_z
            base_z = 0
            poligonos = geometria.geoms if isinstance(geometria, MultiPolygon) else [geometria]
            for p in poligonos:
                try:
...
print(f"Mostrando modelo 3D de los SABs de la opción {option}...")
escena.show()

if dia_horas == 'dia':
    if option == '1':
        escena.export(PATH_sabs + f"modelo_final_{fecha_sel}_SAB2_1.glb")
    elif option == '2':
        escena.export(PATH_sabs + f"modelo_final_{fecha_sel}_SAB2_2.glb")
elif dia_horas == 'hora':
    if option == '1':
        escena.export(PATH_sabs + f"modelo_final_{fecha_sel}_{hora_ini_str}-{hora_fin_str}_SAB2_1.glb")
    elif option == '2':
        escena.export(PATH_sabs + f"modelo_final_{fecha_sel}_{hora_ini_str}-{hora_fin_str}_SAB2_2.glb")
